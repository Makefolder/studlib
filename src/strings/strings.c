...
uint32_t utf8_to_codepoint (const string_t str, int *char_len) {
    ...
    if (num_bytes == 1) {
        ...
        *char_len = 3;
        return ((byte1 & 0x0F) << 12) | (((unsigned char) str[1] & 0x3F) << 6) | ((unsigned char) str[2] & 0x3F);
    } else if (num_bytes == 4) {
        *char_len = 4;
        return ((byte1 & 0x07) << 18) | (((unsigned char) str[1] & 0x3F) << 12) | (((unsigned char) str[2] & 0x3F) << 6) | ((unsigned char) str[3] & 0x3F);
    } else {
